...
-- Return employee_id and their department_id of primaries
-- for one's with only one department return that only one (COUNT = 1)

WITH single_dept_employees as (
    SELECT 
        employee_id    
    FROM Employee
...
WHERE primary_flag = 'Y'
    OR employee_id IN (SELECT * FROM single_dept_employees)